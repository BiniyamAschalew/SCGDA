...
class OPALBase(nn.Module):
    ...
    def __init__(self, features, hidden, classes, dprate=0.0, K=15, cheb_lambda_max=2.0):
        super(OPALBase, self).__init__()
        self.lin1 = nn.Linear(features, hidden)
        self.lin2 = nn.Linear(hidden, classes)


        self.prop1 = ChebProp(K=K)
        self.prop2 = ChebProp(K=K)
        self.prop3 = ChebProp(K=K)
    ...
    def forward(self, data, is_source_domain=True):
        ...
        x = F.dropout(x, p=self.dprate, training=self.training)
        x = self._apply_prop(self.prop3, x, edge_index)
        return x
    ...
    def get_props(self, x, edge_index, is_source_domain=True):
        ...
        x = F.dropout(x, p=self.dprate, training=self.training)
        if is_source_domain:
            x = self._apply_prop(self.prop1, x, edge_index)
        else:
            x = self._apply_prop(self.prop2, x, edge_index)
        return x

    def _apply_prop(self, prop, x, edge_index):

...
class A2GNN(BaseGDA):
    ...
    def fit(self, source_data, target_data):
        ...
        for epoch in range(self.epoch):
            ...
            early_stop = self.early_stop_check(self.a2gnn, result=train_results, epoch=epoch)

            if early_stop == "stop":
                break
            elif early_stop == "save":
                torch.save(self.a2gnn.state_dict(), self.best_model_dir)

        end_time = time.time()
        training_time = end_time - start_time

        self.a2gnn.load_state_dict(torch.load(self.best_model_dir))
        if self.verbose >= 1:

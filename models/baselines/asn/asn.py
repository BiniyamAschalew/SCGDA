...
class ASN(BaseGDA):
    ...
    def __init__(self, config):
        ...
        super(ASN, self).__init__(config)
        assert config["model"]["batch_size"] == 0, 'unsupport for batch training'
        
        self.hid_dim_vae=config["model"]["hid_dim_vae"]
        self.adv_dim=config["model"]["adv_dim"]
        self.lambda_r=config["model"]["lambda_r"]
        self.lambda_d=config["model"]["lambda_d"]
        self.lambda_f=config["model"]["lambda_f"]
    ...
    def fit(self, source_data, target_data):
        ...
        for epoch in range(self.epoch):
            ...
            train_results = self.metrics(source_logits, source_data.y)

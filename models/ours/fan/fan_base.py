...
class FANBase(nn.Module):
    """A2GNN-style backbone with learnable monomial filters."""
    ...
    @staticmethod
    def _build_conv(in_dim: int, out_dim: int):
    ...
    def _to_filter_param(self, filter_param_or_idx, x: torch.Tensor) -> torch.Tensor:
        if torch.is_tensor(filter_param_or_idx):
            ...
            if int(filter_param.numel()) != self.k:
                raise ValueError(
                    f"filter_param length mismatch: expected K={self.k}, got {int(filter_param.numel())}"
                )
            return filter_param

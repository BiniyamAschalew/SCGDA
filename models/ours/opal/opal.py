...
class OPAL(BaseGDA):
    ...
    def forward_model(self, source_data, target_data):
        ...
        source_logits = self.opal.feat_classifier(source_feature, source_data.edge_index)

        target_feature = self.opal.feat_bottleneck(target_data.x, target_data.edge_index, is_source_domain=False)
        target_logits = self.opal.feat_classifier(target_feature, target_data.edge_index)
        ...
        entropy_loss = self.entropy_loss(target_logits)
    ...
    def entropy_loss(self, output, eps: float = 1e-8):

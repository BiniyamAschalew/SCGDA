...
"""SimGDA with spectral filters (Chebyshev) 
with a frozen filter for the target domain and trainable filter for the source domain
along with the MMD loss, we use probe based MMD loss as an additional regularizer 
to align the filter outputs on the source and target topology (for an aligned feature space)"""

class SimGDACheb(BaseGDA):

    def __init__(self, config):
        super(SimGDACheb, self).__init__(config)
        self.config = config
        ...
        self.spectral_type = "cheb" # always using chebyshev filter for this model
    ...
    def _init_dummy_features(self, x_ref):
        
        prob_type = self.config["model"]["probe_type"].lower()
        if prob_type == "gaussian":
            mean = self.config["model"]["probe_mean"]
            std = self.config["model"]["probe_std"]
            init = torch.randn(x_ref.shape, device=x_ref.device, dtype=x_ref.dtype)
            return mean + std * init

        else:
            raise ValueError(f"Invalid probe initialization: {self.config['model']['probe_type']}")
    ...
    def forward_model(self, source_data, target_data):
        ...
        loss += self.probe_mmd_loss(source_features, target_features)

        return loss, source_logits, target_logits

    def probe_mmd_loss(self, source_features, target_features):

        loss = torch.tensor(0.0, device=self.device)
        if self.config["model"].get("probe_mmd_weight", 0) == 0:
            return loss
        
        weight = self.config["model"]["probe_mmd_weight"]
        source_probe = self._init_dummy_features(source_features)
        target_probe = self._init_dummy_features(target_features)

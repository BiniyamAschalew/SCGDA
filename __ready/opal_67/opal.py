...
class OPAL(BaseGDA):
    ...
    def forward_model(self, source_data, target_data):
        ...
        theta_s = self.opal.src_filter.coef
        theta_t = self.opal.tgt_filter.coef


        if self.delta > 0.0:
            # if we align the probes, we don't need theta regularization
            filter_loss = self.filter_alignment(source_data, target_data) 
            loss = loss + filter_loss * self.delta
            theta_reg = 0.0
        else:
            theta_reg = torch.sum(torch.abs(theta_s)) + torch.sum(torch.abs(theta_t))

        theta_loss = F.l1_loss(theta_s, theta_t) + theta_reg
        loss = loss + theta_loss * self.alpha

        source_feature = F.relu(self.opal.lin1(source_data.x))
        target_feature = F.relu(self.opal.lin1(target_data.x))
    ...
    def filter_alignment(self, source_data, target_data):

        # model actual distribution
        with torch.no_grad():
            source_feature = self.opal.lin1(source_data.x)
            target_feature = self.opal.lin1(target_data.x)

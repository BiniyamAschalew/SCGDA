...
class SpectralGNN(nn.Module, _DomainAlignMixin):
    needs_adjacency = True
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if num_layers != 3:
            raise ValueError("SpectralGNN is fixed to depth 3.")
        ...
        self.classifier = nn.Linear(output_dim, num_classes)
        self.spectral_logits = nn.Parameter(torch.zeros(num_layers))
    ...
    def _spectral_weights(self) -> torch.Tensor:
        return F.softmax(self.spectral_logits, dim=0)

    def encode(self, x: torch.Tensor, adj: torch.Tensor | None = None) -> torch.Tensor:
        if adj is None:
            raise ValueError("SpectralGNN requires adjacency matrix for propagation")
        ...
        for layer in self.layers:
            message = a @ h
            next_h = F.relu(layer(message))
            h = self.dropout(h + next_h)
            hidden_states.append(h)

        weights = self._spectral_weights()
        stacked = torch.stack(hidden_states, dim=0)
        z = torch.einsum("l, lnd -> nd", weights, stacked)
        return self.output_projection(F.relu(z))

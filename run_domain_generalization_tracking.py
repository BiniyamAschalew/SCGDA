...
def main():
    parser = argparse.ArgumentParser(
        description="Track how a source-trained GNN generalizes to all domains during training."
    )
    ...
    parser.add_argument(
        "--out-root",
        type=str,
        default="./__saved__/analysis/airport_domain_generalization_tracking",
    )
    ...
    if requested_dataset == "all":
        dataset_keys = ["airport", "blog", "citation", "twitch"]
        ...
        if requested_dataset not in EXPERIMENT_SETTINGS:
            raise ValueError(f"Unknown dataset setting '{args.dataset}'. Available: {sorted(EXPERIMENT_SETTINGS) + ['all']}")
    ...
    for dataset_key in dataset_keys:
        ...
        dataset_run_root = run_root / dataset_key if len(dataset_keys) > 1 else run_root
        dataset_case_dirs[dataset_key] = []

        for source in sources:
            print(f"Running scenario: {dataset_key} source={source} eval={eval_domains} seeds={seeds}")
            target_domains = [domain for domain in eval_domains if domain != source]
            case_dir = dataset_run_root / f"{source}_to_{'-'.join(target_domains) if target_domains else 'none'}"
            out_dir = run_multi_seed_experiment(
                dataset=dataset_key,
                source=source,
            ...
            result_dirs.append((dataset_key, source, Path(out_dir)))
            dataset_case_dirs[dataset_key].append(Path(out_dir))

        build_dataset_rollup(
            dataset=dataset_key,
            dataset_out_dir=dataset_run_root,
            case_dirs=dataset_case_dirs[dataset_key],
        )

    print("\nCompleted runs:")
    print(f"- shared run root: {run_root}")
    for dataset_key, source, out_dir in result_dirs:
        print(f"- {dataset_key} source={source}: {out_dir}")
